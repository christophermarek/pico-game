...
static const uint8_t FONT_DATA[(FONT_LAST - FONT_FIRST + 1) * FONT_COLS] = {
...
void font_draw_char(char c, int x, int y, uint16_t color, int scale) {
    ...
    const uint8_t *glyph = &FONT_DATA[idx * FONT_COLS];

    for (int col = 0; col < FONT_COLS; col++) {
        uint8_t bits = glyph[col];
        for (int row = 0; row < FONT_ROWS; row++) {
            if (bits & (1u << row)) {
                hal_fill_rect(x + col * scale,
                              y + row * scale,
                              scale, scale, color);
            }
        }
...
void font_draw_str(const char *s, int x, int y, uint16_t color, int scale) {
    int cx = x;
    while (*s) {
        font_draw_char(*s++, cx, y, color, scale);
